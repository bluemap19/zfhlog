...
def multifractal_analysis(image_array: np.ndarray, q_range: float = 5.0, q_step: float = 0.1, img_size:tuple = None, box_sizes:list=[2, 4, 8, 16, 32, 64, 128]) -> Dict:
    ...
    analysis_results = {
        'q_values': q_values_storage,  # q参数序列
        'singularity_exponents': singularity_exponents,  # 奇异指数α(q)
        'multifractal_spectrum': multifractal_spectrum,  # 多重分形谱f(α)
        'generalized_dimensions': generalized_fractal_dimensions,  # 广义分形维数D_q
        'mass_exponents': mass_exponent_tau,  # 质量指数τ(q)
        'alpha_regression_quality': alpha_regression_r2,  # α(q)拟合质量
        'spectrum_regression_quality': f_spectrum_regression_r2,  # f(α)拟合质量
        'total_scale_levels': maximum_scale_levels,  # 总尺度级数
        'processed_image_size': standardized_image_size,  # 处理后的图像尺寸
        'scale_values': scale_sequence,  # 尺度序列
        'partition_function': partition_function_values,  # 配分函数χ(q,ε)
