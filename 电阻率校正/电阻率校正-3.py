...
def window_process_gauss_scaling(df, PRED_GAUSS_SETTING={}, LOG_USE=False,
    ...
    for i in range(windows_num):
        ...
        if PRED_GAUSS_SETTING:
            ...
            if LOG_USE:
                df_window['R_gauss_log'], stats = scale_gaussian(source_data=df['R_temp_sub'], target_data=df['R_real'],
                                                          return_stats=True)
                df_window['R_gauss'] = np.power(np.e, df['R_gauss_log'])
            else:
                df_window['R_gauss'], stats = scale_gaussian(source_data=df['R_temp_sub'], target_data=df['R_real'],
                                                      return_stats=True)
...
def fit_r_pred_window_gauss_scaling(df, PRED_GAUSS_SETTING={}, offset_function='linear', LOG_USE=False):
    # 确保输入包含所需列
    if PRED_GAUSS_SETTING:
        required_cols = ['R_temp', 'TEMP', 'R_gauss']
    else:
    ...
    df = window_process_gauss_scaling(df, PRED_GAUSS_SETTING, LOG_USE=LOG_USE,
                                      window_work_length=200, windows_step=100, windows_view=1.2)
...
if __name__ == '__main__':
    ...
    df = fit_r_pred_window_gauss_scaling(df, PRED_GAUSS_SETTING={}, offset_function='power', LOG_USE=True)
