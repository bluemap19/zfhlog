...
def fit_r_pred_window_gauss_scaling(df, PRED_GAUSS_SETTING={}, offset_function='power', gauss_windows=200, windows_step=10):
    # 确保输入包含所需列
    if PRED_GAUSS_SETTING:
    ...
    df = window_process_gauss_scaling(df, PRED_GAUSS_SETTING,
                                      window_work_length=gauss_windows, windows_step=windows_step, windows_view=1.2)
...
if __name__ == '__main__':
    ...
    path_logging = search_files_by_criteria(search_root=r'F:\桌面\电阻率校正\SOF003',
    ...
    gauss_windows = 200
    df = fit_r_pred_window_gauss_scaling(df, PRED_GAUSS_SETTING={}, offset_function='tempture', gauss_windows=gauss_windows, windows_step=10)
    # df = fit_r_pred_window_gauss_scaling(df, PRED_GAUSS_SETTING={}, offset_function='linear', gauss_windows=gauss_windows, windows_step=10)
