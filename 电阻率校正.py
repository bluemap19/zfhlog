...
def fit_r_pred(df, PRED_GAUSS_SETTING={}, offset_function='linear'):
    ...
    if PRED_GAUSS_SETTING:
        required_cols = ['R_temp', 'TEMP', 'R_pred']
    else:
        required_cols = ['R_temp', 'TEMP', 'R_real', 'R_pred']
    ...
    if PRED_GAUSS_SETTING:
        df['R_pred'], stats = scale_gaussian_by_config(source_data=df['R_temp_sub'], target_data_config=PRED_GAUSS_SETTING, return_stats=True)
    else:
        df['R_pred'], stats  = scale_gaussian(source_data=df['R_temp_sub'], target_data=df['R_real'], return_stats=True)
...
if __name__ == '__main__':
    ...
    time_str = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    df.to_csv(path_logging.replace('02-第9井次  坨73-斜13井--resampled-0.1m_logging', f'坨73-斜13井-{time_str}.csv'), index=False, float_format='%.4f', quoting=csv.QUOTE_NONE)
