...
from DongYing_WangZiMing.linear_fitting_temp import robust_linear_fit
...
if __name__ == '__main__':
    ...
    for path in path_list_target:
        ...
        print('IMAGE_GREY shape :{} and set depth shape:{}'.format(IMAGE_GREY.shape, DEPTH_TEMP.shape))

        # 这个很重要，如果选的区域较大，就会有比较大的噪声影响，比较小的话，拟合又容易不准
        # IMAGE_GREY = IMAGE_GREY[:, width//5*2:width//5*3]
        ...
        for j in range(1, DF_GREY.shape[1]):
            ...
            correct_shift, _ = denoising_depth_correction(df_depth_correct, depth_col=0, base_col=1, process_col=2,
                                       search_range=0.5, coarse_res=0.1, fine_res=0.01,
        ...
        model, slope, r2, inliers = robust_linear_fit(
            ...
            min_samples=0.4, residual_threshold=0.1, plot=False
        )
        slope/=depth_resolution
        print('file {} use slope as {}, it\'s r2 is {}'.format(path.split('\\')[-1].split('.')[0], slope, r2))

        # 进行图像的拉伸、菱形拉伸
        IMAGE_SHEER = shear_image(IMAGE_SCALED, slope)
        ...
        # plt.axis('off')
        # plt.subplot(1, 3, 2)
        # plt.imshow(IMAGE_SHEER)
        # plt.title(f"斜率 slope={slope:.4f}\n{slope/0.0025:.4f}")
        # plt.axis('off')
        # plt.subplot(1, 3, 3)
        # plt.imshow(IMAGE_STRIPE)
        # plt.title(f"原始图像 ({height}×{width})")
        # plt.axis('off')
        # # 解决中文显示问题
        ...
        # plt.tight_layout()
        # plt.savefig(PATH_FOLDER+ '\\target\\' + path.split('\\')[-1].split('.')[0] + '.png')
        # plt.show()
