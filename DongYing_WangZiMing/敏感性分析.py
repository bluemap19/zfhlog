...
if __name__ == '__main__':
    ...
    windows_length = [40, 60, 80, 100, 120, 140]
    ...
    path_table_target = LG.search_target_file_path(well_name='well_texture_all', target_path_feature=['LITHO_TYPE'], target_file_type='table')
    print(LG.get_table_3_all_data(well_names=['well_texture_all'], file_path={'well_texture_all':path_table_target}))
    print(LG.get_all_table_replace_dict(well_names=['well_texture_all']))

    replace_dict = {'亮晶': 0, '隐晶': 1, '层界面': 2, '块状': 3}
    ...
    for i in windows_length:
        ...
        AUC_tree_num_dict = {}
        Imp_tree_num_dict = {}
        path_logging_target = LG.search_target_file_path(well_name='well_texture_all', target_path_feature=['texture', f'_{i}'], target_file_type='logging')
        path_table_target = LG.search_target_file_path(well_name='well_texture_all', target_path_feature=['Litho_Type'], target_file_type='table')
        print('get data from path:{}, get table from:{}'.format(path_logging_target, path_table_target))

        data_input = LG.combined_all_logging_with_type(well_names=['well_texture_all'],
