...
if __name__ == '__main__':
    ...
    WELL_TEST = DATA_WELL(path_folder=r'F:\桌面\算法测试-长庆数据收集\logging_CSV\禄探')
    # WELL_TEST = DATA_WELL(path_folder=r'F:\桌面\算法测试-长庆数据收集\logging_CSV\樊页2HF')
    path_list_stat, path_list_dyna = WELL_TEST.get_FMI_path_list()
    WELL_TEST.data_FMI_init(path_stat=path_list_stat[0], path_dyna=path_list_dyna[0])
    Data_all = WELL_TEST.get_fmi_data(fmi_stat_key=path_list_stat[0], fmi_dyna_key=path_list_dyna[0])
    print(Data_all['DYNA'].shape, Data_all['STAT'].shape, Data_all['DEPTH_DYNA'].shape)
    taaaaaa = WELL_TEST.get_fmi_texture(path_saved=r'F:\桌面\算法测试-长庆数据收集\logging_CSV\禄探\\禄探_TEXTURE.csv')
    # taaaaaa = WELL_TEST.get_fmi_texture(path_saved='F:\\桌面\\算法测试-长庆数据收集\\logging_CSV\\樊页2HF\\樊页2HF_TEXTURE.csv')
    print(taaaaaa.shape)