...
class WellLogVisualizer:
    ...
    def _validate_nmr_data(self, nmr_dict) -> None:
        """验证NMR数据的结构和完整性 - 增强验证"""
        if nmr_dict is None:
            logger.info("无NMR数据")
            return

        if isinstance(nmr_dict, dict):
            # 新格式：FMI式数据结构
            nmr_dict = self._validate_nmr_data_2(nmr_dict)
            return nmr_dict
        elif isinstance(nmr_dict, list):
            # 旧格式：深度字典结构
            nmr_dict = self._validate_nmr_data_1(nmr_dict)
            return nmr_dict
        else:
            raise ValueError(f"不支持的NMR数据格式: {type(nmr_dict)}")

    def _validate_nmr_data_1(self, nmr_dict: Dict) -> None:
        """验证旧格式NMR数据"""
        invalid_keys = []
        ...
        # 移除空的数据道
        nmr_dict = [nmr_data for nmr_data in nmr_dict if nmr_data and len(nmr_data) > 0]
    ...
    def _validate_nmr_data_2(self, nmr_dict: List) -> None:
        """验证新的NMR数据格式"""
        if nmr_dict is None:
            logger.info("无NMR数据")
            return
        ...
        # 将新格式转换为旧格式，保持向后兼容
        return self._convert_nmr_format(nmr_dict)

    def _convert_nmr_format(self, nmr_dict: Dict[str, Any]) -> None:
        """将新格式NMR数据转换为旧格式"""
        NMR_dict_NEW = []
        ...
        for i, nmr_array in enumerate(nmr_dict['nmr_data']):
            ...
            for j, depth in enumerate(depth_data):
                ...
                # 归一化振幅
                if nmr_spectrum.max() > 0:
                    normalized_amplitude = nmr_spectrum / nmr_spectrum.max()
                else:
                    normalized_amplitude = nmr_spectrum

                nmr_old_format[float(depth)] = {
                    'NMR_X': t2_values,
                    'NMR_Y': normalized_amplitude,
                }

            NMR_dict_NEW.append(nmr_old_format)

        logger.info("NMR数据格式转换完成，转换为%d个数据道", len(NMR_dict_NEW))
        return NMR_dict_NEW
    ...
    def visualize(self,
        ...
        try:
            ...
            fmi_dict = self._validate_fmi_data(fmi_dict)
            NMR_dict = self._validate_nmr_data(NMR_dict)
