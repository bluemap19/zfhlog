...
class EnhancedWellLogCache:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        """获取缓存统计"""
        data_hit_rate = (self.stats['data_hits'] / (self.stats['data_hits'] + self.stats['data_misses']) * 100
                         if (self.stats['data_hits'] + self.stats['data_misses']) > 0 else 0)

        fmi_hit_rate = (self.stats['fmi_hits'] / (self.stats['fmi_hits'] + self.stats['fmi_misses']) * 100
                        if (self.stats['fmi_hits'] + self.stats['fmi_misses']) > 0 else 0)

        compression_ratio = (self._fmi_compression_stats['original_size'] /
        ...
        return {
            ...
            'memory_saved_mb': (self._fmi_compression_stats['original_size'] -
    ...
    def clear_cache(self):
        ...
        self.stats = {'data_hits': 0, 'data_misses': 0, 'fmi_hits': 0, 'fmi_misses': 0}
        self._fmi_compression_stats = {'compressed_size': 0, 'original_size': 0}
...
class WellLogVisualizer:
    ...
    def _calculate_subplot_count(self) -> int:
        ...
        n_NMR = len(self.NMR_dict) if self.NMR_dict else 0

        return n_curve_panels + n_type_panels + n_fmi_panels +n_NMR
    ...
    def _get_visible_NMR_data(self, top_depth: float, bottom_depth: float) -> Dict[int, Dict[float, Dict[str, Any]]]:
        ...
        返回:
            Dict: {nmr_index: {depth: nmr_data}} 每个NMR道的可见数据
        """
        visible_nmr_data = {}
    ...
    #     nmr_data_group = self.NMR_dict[nmr_index]
    #
    #     # 创建标题框
    ...
    def _plot_nmr_panel(self, ax: plt.Axes, nmr_index: int, panel_index: int) -> None:
        ...
        # T2核磁谱设置
        # ax.set_xscale('log')  # T2时间使用对数坐标
        # ax.set_xlim(0.1, 1000)  # 典型的T2时间范围
        # ax.grid(True, alpha=0.3)
        
        # 这个则是正常谱设置
        ax.set_xlim(0, 100)  # 典型的T2时间范围
        ax.grid(True, alpha=0.3)
    ...
    def _initialize_nmr_object_pool(self, nmr_plot: Dict[str, Any]) -> None:
        ...
        for i in range(max_pool_size):
            line, = ax.plot([], [], 'b-', linewidth=0.8, alpha=0.7, visible=False)
            nmr_plot['line_pool'].append(line)

        # 预创建填充对象
        for i in range(max_pool_size):
            fill = ax.fill_between([], [], [], alpha=0.3, color='blue', linewidth=0, visible=False)
            nmr_plot['fill_pool'].append(fill)

        # 预创建文本对象
        for i in range(max_pool_size):
            text = ax.text(0, 0, '', fontsize=6, va='center', ha='left', alpha=0.7, visible=False)
            nmr_plot['text_pool'].append(text)
    ...
    #
    #         # 清除之前的谱线和文本
    #         for line in nmr_plot['lines']:
    #             line.remove()
    #         for text in nmr_plot['texts']:
    #             text.remove()
    #         for fill in nmr_plot.get('fills', []):
    #             fill.remove()
    #         nmr_plot['lines'] = []
    #         nmr_plot['texts'] = []
    #         nmr_plot['fills'] = []
    ...
    #             max_NMR_Y = 1  # 避免除零
    #
    #         scale_factor = 25/len(nmr_data_group)
    #
    #         # 为每个深度的NMR数据绘制谱图
    #         for depth, nmr_data in nmr_data_group.items():
    ...
    #
    #             # 绘制填充区域（从基线到谱线）
    #             fill = ax.fill_between(NMR_X, baseline, y_values,
    #                                    alpha=0.3,  # 填充透明度
    #                                    color='blue',  # 填充颜色
    #                                    linewidth=0)  # 填充边框宽度
    #             nmr_plot['fills'].append(fill)
    #
    #             # 绘制NMR谱线
    #             line, = ax.plot(NMR_X, y_values, 'b-', linewidth=0.8, alpha=0.7)
    #             nmr_plot['lines'].append(line)
    #
    #             # 在谱图上方添加深度标签
    #             if len(NMR_X) > 0:
    #                 text = ax.text(NMR_X[-1] * 1.1, depth, f'{depth:.1f}m',
    #                                fontsize=6, va='center', ha='left', alpha=0.7)
    #                 nmr_plot['texts'].append(text)
    #
    #         # 设置坐标轴范围
    #         if nmr_data_group:
    #             # 获取所有T2值范围
    #             all_X_min = min([data['NMR_X'].min() for data in nmr_data_group.values()])
    #             all_X_max = max([data['NMR_X'].max() for data in nmr_data_group.values()])
    #
    #             ax.set_xlim(all_X_min, all_X_max * 1.2)
    ...
    def _update_nmr_display(self, top_depth: float, bottom_depth: float) -> None:
        ...
        for nmr_plot in self.nmr_plots:
            ...
            for i, (depth, nmr_data) in enumerate(nmr_data_group.items()):
                ...
                # 创建新的填充
                new_fill = ax.fill_between(NMR_X, baseline, y_values, alpha=0.3, color='blue', linewidth=0)
                nmr_plot['fill_pool'][i] = new_fill
                nmr_plot['active_fills'].append(new_fill)

                # 更新文本
                if len(NMR_X) > 0:
                    text.set_text(f'{depth:.2f}m')
                    text.set_position((NMR_X[-1] * 1.1, depth))
                    text.set_visible(True)
                    nmr_plot['active_texts'].append(text)
    ...
    def _hide_all_active_objects(self, nmr_plot: Dict[str, Any]) -> None:
        ...
        # 隐藏文本
        for text in nmr_plot['active_texts']:
            text.set_visible(False)
        nmr_plot['active_texts'] = []
    ...
    def _expand_object_pool(self, nmr_plot: Dict[str, Any], new_size: int) -> None:
        ...
        # 扩展文本对象池
        for i in range(current_size, new_size):
            text = ax.text(0, 0, '', fontsize=6, va='center', ha='left',
    ...
    def _update_display(self) -> None:
        ...
        self._update_nmr_display(top_depth, bottom_depth)  # NMR核磁谱
...
if __name__ == "__main__":
    ...
    # 生成更真实的NMR测试数据
    NMR_DICT1 = generate_nmr_data(depth_range=[depth_fmi[0], depth_fmi[-1]], num_points=10)
    print(NMR_DICT1)
    NMR_DICT2 = generate_nmr_data(depth_range=[depth_fmi[0], depth_fmi[-1]], num_points=10)
    print(NMR_DICT2)
